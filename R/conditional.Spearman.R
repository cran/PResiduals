...
#' 
#' \code{conditional.partial.Spearman} computes the partial Spearman's rank correlation between variable \var{X} and variable \var{Y} adjusting for variable \var{Z} conditional on \var{Zc}. 
...
#' \code{conditional.partial.Spearman} is called.
#' 
...
#' 
#' @return object of \samp{conditional.Spearman} class.
#' @export
#' @seealso \code{\link{print.conditional.Spearman}},\code{\link{print.conditional.Spearman}}
#' @examples
#' data(PResidData)
#' library(rms)
#' #### fitting cumulative link models for both Y and W
#' result <- conditional.partial.Spearman(c|y~ x + w, conditional.by="w",
...
conditional.partial.Spearman <- function(formula, conditional.by, data,
    ...
    ans$est <- result
    ans <- structure(ans, class="conditional.Spearman")
