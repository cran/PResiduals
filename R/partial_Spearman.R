...
#'
#' \code{partial_Spearman} computes the partial Spearman's rank correlation
#' between variable \var{X} and variable \var{Y} adjusting for other
#' variables, \var{Z}.  The basic approach involves fitting a specified model
#' of \var{X} on \var{Z}, a specified model of \var{Y} on \var{Z}, obtaining
#' the probability-scale residuals from both models, and then calculating
#' their Pearson's correlation.  \var{X} and \var{Y} can be any orderable
#' variables, including continuous or discrete variables.  By default,
#' \code{partial_Spearman} uses cumulative probability models (also referred
#' as cumulative link models in literature) for both \var{X} on \var{Z} and
#' \var{Y} on \var{Z} to preserve the rank-based nature of Spearman's
...
#' \var{Y} adjusting for \var{Z}, \samp{formula} is specified as
#' \code{\var{X} | \var{Y} ~ \var{Z}}.  This indicates that models of
#' \code{\var{X} ~ \var{Z}} and \code{\var{Y} ~ \var{Z}} will be fit.
#'
...
#'
#' @param fit.x,fit.y the fitting functions used for the models of X or Y on
#'     Z.  The default function is \samp{orm}, which fits cumulative
#'     probability models for continuous or discrete ordinal variables. Other
#'     options include \samp{lm}, which fits linear regression models and
#'     obtains the probability-scale residuals by assuming normality;
#'     \samp{lm.emp}, which fits linear regression models and obtains the
#'     probability-scale residuals by empirical ranking; \samp{poisson},
#'     which fits Poisson models for count variables; \samp{nb}, which fits
#'     negative binomial models for count variables; and \samp{logistic},
#'     which fits logistic regression models for binary variables.
#'
...
#'     environment from which \code{partial_Spearman} is called.
#'
